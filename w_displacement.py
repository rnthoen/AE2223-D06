...
def variable_load_curve_data(cycle_number_list, df_separated, df_data, X_position, Y_position, variable):
    ...
    for cycle_number in cycle_number_list:
        for xs in X_position:
            ...
            while i <= end_count:
                if i%2 == 0:
                    variable_position = interpolate_panel(df_data[1],i,xs,Y_position[idx0],variable)
                    ...
                    variable_median = df_data[1][variable].iloc[idx2]
                    variable_median_list.append(variable_median)
                    for k in idx1:
                        variable2_list.append(df_data[1][variable].iloc[k])
                    variable_min_list.append(min(variable2_list))
                    variable_max_list.append(max(variable2_list))
                    variable2_list = []
                i += 1

            # format in a return array
            result.append([cycle_number, load_list, variable_min_list, variable_median_list, variable_max_list,variable_position_list])

    return result

cycle_number_list = [500, 60500]
...
plot_data = variable_load_curve_data(cycle_number_list, df_separated, df_data, x_middles, y_middles, variable)
...
for count, cycle in enumerate(cycle_number_list):
    for n in range(len(x_middles)):
        ...
        axs[count].set_title(f'variable at x = {x_middles}, y = {y_middles} vs. Load')
...
for ax in axs.flat:
    ax.set(xlabel='Load [kN]', ylabel=variable)

#fig.suptitle(f'variable-Load Analysis of {select_specimen[0][5:9]}')
plt.show()
